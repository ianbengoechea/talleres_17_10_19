...
import NoticiaConfig from './NoticiaConfig';

export default class NoticiaService {

  static fetchAllNotcias() {
    return new Promise(async (resolve, reject) => {
      try {
        const endpoint = NoticiaConfig.fetchAllNoticias;
        const response = await NoticiaConfig.APIConnector.get(endpoint);
        resolve(response);
      } catch (err) {
        reject(err);
      }
